		...
		/*校验一些输入表单是否为空*/
  		...
  		if(score.value == ""){
			...
			if(confirm("提交后成果将无法修改，确认通过?")){  
				...
				form.setAttribute("action", "<%=projectPath%>/services/EduReformProjectUpload?next=pass");
				form.removeAttribute("enctype");
				form.removeAttribute("target");
				//form.submit();			
			}else{
		...
		/*校验一些输入表单是否为空*/
  		...
  		if(score.value == ""){
			...
			if(confirm("确认将成果设置为不通过?")){  
				...
				var checked = document.getElementById("checked");
				checked.value-=1;
				form.setAttribute("action", "<%=projectPath%>/services/EduReformProjectUpload?next=unpass");
				form.removeAttribute("enctype");
				form.removeAttribute("target");
				form.submit();			
			}else{
