									...
									<%}%>
										<div class="form-item">
											...
											<div class="moco-control-input">
	                              				  <input type="text" name="patentName" id="patentName" autocomplete="off" class="moco-form-control" value="<%=p.getName() %>" placeholder="请输入专利、软件著作名称..."<%if(request.getParameter("state").equals("VIEW")){%>readonly="readonly"<%}%>>
										...
										<div class="form-item">
											<label>类型:</label>
											<div class="moco-control-input">
                              				  <select class="moco-form-control rlf-select" name="category" hidefocus="true" id="category" data-validate="require-select" <%if(request.getParameter("state").equals("VIEW")){%> onfocus="this.defaultIndex=this.selectedIndex;" onchange="this.selectedIndex=this.defaultIndex;"<%}%>>
										...
										<div class="form-item">
											...
											<div class="moco-control-input">
	                              				  <input type="text" name="patentNum" id="patentNum" autocomplete="off" class="moco-form-control" value="<%=p.getPatentNum() %>" placeholder="请输入专利编号..." <%if(request.getParameter("state").equals("VIEW")){%>readonly="readonly"<%}%>>
	                               				 <div class="rlf-tip-wrap errorHint color-red"></div>
	                           				 </div>
										</div>
										<div class="form-item">
											...
											<div class="moco-control-input">
	                              				  <input type="text" name="patentHolder" id="patentHolder" autocomplete="off" class="moco-form-control" value="<%=p.getPatentHolder() %>" placeholder="请输入专利发明人..." <%if(request.getParameter("state").equals("VIEW")){%>readonly="readonly"<%}%>>
	                               				 <div class="rlf-tip-wrap errorHint color-red"></div>
	                           				 </div>
										</div>
										<div class="form-item">
											...
											<div class="moco-control-input">
	                              				  <input type="text" name="patentDate" id="patentDate" autocomplete="off" class="moco-form-control" value="<%if(p.getPatentDate()!=null){%><%=p.getPatentDate()%><%}%>" <%if(request.getParameter("state").equals("VIEW")){%>readonly="readonly"<%}%>>
	                               				 <div class="rlf-tip-wrap errorHint color-red"></div>
	                           				 </div>
										</div>
										<%if(request.getParameter("AchievementId")!=null){%>
		...
		/*校验一些输入表单是否为空*/
  		...
  		if(score.value == ""){
			...
			if(confirm("提交后成果将无法修改，确认通过?")){  
				...
				form.setAttribute("action", "<%=projectPath%>/services/PatentUpload?next=pass");
				form.removeAttribute("enctype");
				form.removeAttribute("target");
				form.submit();			
			}else{
		...
		/*校验一些输入表单是否为空*/
  		...
  		if(score.value == ""){
			...
			if(confirm("确认将成果设置为不通过?")){  
				...
				var checked = document.getElementById("checked");
				checked.value-=1;
				form.setAttribute("action", "<%=projectPath%>/services/PatentUpload?next=unpass");
				form.removeAttribute("enctype");
				form.removeAttribute("target");
				form.submit();			
			}else{
